...
import (
	...
	util2 "github.com/go-spring/spring-base/internal/util"
)

var converters = map[reflect.Type]util2.Converter{}
...
func RegisterConverter(fn util2.Converter) {
	t := reflect.TypeOf(fn)
	if !util2.IsConverter(t) {
...
func injectAttribute(tag string, fv reflect.Value, ft reflect.StructField, node *Node) error {
	...
	if fn := converters[ft.Type]; fn != nil {
		...
		if !out[1].IsNil() {
			...
			return util2.Wrapf(err, code.FileLine(), "inject error")
		}
		fv.Set(out[0])
		return nil
	}
	...
	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		...
		return util2.Wrapf(err, code.FileLine(), "inject %s error", ft.Name)
...
func injectElement(tag string, fv reflect.Value, ft reflect.StructField, node *Node) error {
	...
	for _, c := range node.Children {
		...
		if !ok {
			...
			return util2.Wrap(err, code.FileLine(), "inject element")
		}
		if p.Type != elemType {
			continue
		}
